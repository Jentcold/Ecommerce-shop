...
const EMPTY_FORM = {
  ...
  is_featured: false,
  image_urls: '',
  sizes: [], // Handled dynamically in the product payload
}

const PRESET_SIZES = ['S', 'M', 'L', 'XL', 'XXL']

function CategoryField({ value, onChange, categories, T }) {
  const isCustom =
    value === '__custom__' || (value && !categories.includes(value))
  ...
  return (
    <div className="space-y-2">
      <select
        ...
        required={!showCustom}
        className="w-full border border-zinc-200 rounded px-3 py-2 text-sm outline-none focus:border-zinc-400 bg-white"
      >
